...
const IconMorph = ({ initialIcon, toggleIcon, isToggled, setIsToggled }: IconMorphProps) => {
  ...
  return (
    <button
      className={`flex justify-center items-center font-medium bg-custom-light/5 w-12 h-12 rounded-full text-sm border border-custom-light/10 backdrop-blur`}
      onClick={handleClick}
      key={'icon-morph'}
    >
      <motion.div
    ...
    <AnimatePresence key={'menu-modal-animate-presence'}>
      {isToggled && (
        <motion.div
          ...
          className="absolute flex top-full w-32 mt-2 p-4 gap-y-4 right-0 flex-col justify-center items-start font-medium bg-custom-light/5 rounded-xl text-sm border border-custom-light/10 backdrop-blur"
          initial={{
            opacity: 0,
            scale: 0,
            originX: 1,
