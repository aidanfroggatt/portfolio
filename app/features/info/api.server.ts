...
export const getInfoPageData = async () => {
  const [aboutData, experienceData, educationData, involvementData, awardsData] = await Promise.all(
    [
      ...
      db.query.experiences.findMany({
        where: (t, { eq }) => eq(t.type, 'education'),
        orderBy: (t, { desc }) => [desc(t.startDate)],
      }),
      db.query.experiences.findMany({
  ...
  return { aboutData, experienceData, educationData, involvementData, awardsData };
};
