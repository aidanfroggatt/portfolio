...
export async function loader({ params }: LoaderFunctionArgs) {
  ...
  let nextProject = work.find(
    (item) => item.index === currentProject.index + 1
  );

  // If no next project, wrap around to the first project
  if (!nextProject) {
    nextProject = work.reduce(
...
export const meta: MetaFunction = ({ data }) => {
  ...
  return [
    {
      title: `Aidan Froggatt — ${
        projectInfo ? projectInfo.title : "Work Item"
      }`,
    },
    {
      name: "description",
          ...
          <div className="flex flex-wrap md:flex-row w-full justify-evenly md:justify-start gap-8 items-center">
            {projectInfo.overview?.technologies.map((technology, index) => (
              <div
                key={index}
                className="icon-link"
                style={{ "--link-color": projectInfo.color }}
              >
  ...
  return (
    <section id="work-item-highlights">
      <div
        ...
        style={{ "--highlight-card-accent-color": projectInfo.color }}
      >
        <div className="flex flex-col justify-between items-center gap-y-4">
          ...
          <h5 className="text-center">
            {projectInfo?.highlights?.description}
          </h5>
        </div>
        {projectInfo.highlights?.items.map(
        ...
        >
          <div className="flex flex-col gap-y-2">
            ...
            {contentSection.title && (
              <h2 className="text-shadow-mobile md:text-shadow">
                {contentSection.title}
              </h2>
            )}
          </div>
          ...
          <div className="flex flex-col gap-y-8 md:gap-y-16">
            {contentSection.assets &&
              contentSection.assets.map(
                (asset: ImageAsset | VideoAsset | PDFAsset, index) => (
                  <div
                    key={index}
                    className="relative w-full flex flex-col items-center"
                  >
