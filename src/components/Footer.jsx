...
const Footer = () => {
    ...
    return (
        <div
            className="relative bottom-0 bg-custom-dark text-custom-light border-t border-custom-light border-opacity-50 h-80 justify-evenly items-center flex flex-row py-20 px-40 gap-x-32">
            ...
            <div className="h-full w-full flex flex-col justify-between items-end">
                ...
                <div className="flex flex-col h-full w-full items-end justify-end">
                    <div className="text-sm text-custom-light text-opacity-50">Last updated by {getFirstWord(lastCommit.author)}, {convertDateFormat(lastCommit.time)}</div>
                </div>
            </div>
        </div>
    )
